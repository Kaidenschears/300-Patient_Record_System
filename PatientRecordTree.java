...
public class PatientRecordTree {
  ...
  public boolean isEmpty() {
    return size == 0; // a default return statement added to let this code compile.
  }
  ...
  public static boolean addPatientRecordHelper(PatientRecord newRecord, PatientRecordNode current) {
    int result = newRecord.compareTo(current.getPatientRecord());
    if (result < 0) {
  ...
  public boolean addPatientRecord(PatientRecord newRecord) {
    if (isEmpty()) { // Add newRecord to an empty PatientRecordTree
      ...
      boolean temp = addPatientRecordHelper(newRecord, root);
      if (temp == true)
        size++;
      return temp;
  ...
  private PatientRecord lookupHelper(PatientRecord findRecord, PatientRecordNode current) {
    if (current == null || findRecord == null) {
      throw new NoSuchElementException("This Patient Record is not on file.");
    }
    int result = findRecord.compareTo(current.getPatientRecord());
    if (result == 0) {
  ...
  public static int heightHelper(PatientRecordNode current) {
    ...
    int y = 0;
    if (current != null) {
      y = 1 + heightHelper(current.getLeftChild());
      x = 1 + heightHelper(current.getRightChild());
    }
    if (x > y)
      return x;
    else
